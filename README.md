...
| `GET` | `/random` | Получение одного или нескольких случайных действий. |
| `POST` | `/add/{action}` | Добавление нового действия. |
| `GET` | `/all` | Просмотр всех доступных действий. |
| `DELETE` | `/del/{index}` | Удаление действия по его порядковому номеру. |
